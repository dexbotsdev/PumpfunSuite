        ...
        const onAcceptedBundle = async (bundleResult, bundleId) => {
            ...
            if (resbundleId == bundleId) {
                ...
                if (isRejected && !accepted) {
                    ...
                    const updateData = { ...tokenMeta, bundleStatus: 'FAILED',reason:new String(bundleResult.rejected.simulationFailure.msg.message) };
                    const updatedRecord = await update(updateData, 'TokenMeta');
                    const updateJobData = { ...tokenMeta, bundlerStatus: 'FAILED',reason:new String(bundleResult.rejected.simulationFailure.msg.message) };
                    const updatedJobRecord = await update(updateJobData, 'JobMeta');
                }
            }
        }
